...
Thanks for your interest in improving whois.api.airat.top. Contributions are welcome, including bug reports, docs improvements, and API behavior refinements.
...
2. Create a feature branch that describes your work (for example, `feature/improve-registrar-parsing`).
...
- [ ] `GET /` and `GET /json` return WHOIS/RDAP JSON for valid `domain`.
- [ ] `GET /text` returns compact plain-text summary.
- [ ] `GET /yaml` and `GET /xml` return valid formatted output.
- [ ] Invalid or missing `domain` returns documented `400` error.
